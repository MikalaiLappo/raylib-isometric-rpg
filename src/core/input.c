...
void ProcessInput(Game* game) {
    ...
    Vector2 mouseWorld =
        ScreenToWorld((Vector2){mouse.x - game->viewOffset.x, mouse.y - game->viewOffset.y}, game->tilemap.tileSize);
        ...
        if (IsModeButtonClicked(&game->ui, mouse, game->state, &newMode)) {
            ...
            if (game->enemyHover && IsInAttackRange(game->state, &game->player, &game->tilemap, mouseX, mouseZ)) {
                ...
                if (damage > 0) {
                    ...
                    char    damageText[8];
                    snprintf(damageText, sizeof(damageText), "-%d", damage);
                    AddFloatingText(game, screenPos, damageText, RED);
                    printf("Dealt %d damage to enemy!\n", damage);
                    if (enemies[game->hoverEnemyIdx].alive == false) {
    ...
    if (game->state == IDLE && !game->player.isMoving) {
        ...
        if (moved && nx >= 0 && nx < game->tilemap.width && nz >= 0 && nz < game->tilemap.height) {
            if (!IsTileOccupied(nx, nz))
                MovePlayerToTile(&game->player, nx, nz);
        }
    }
