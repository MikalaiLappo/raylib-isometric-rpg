...
void SpawnEnemy(TileMap* map, Vector2 avoidPos) {
    ...
    while (!found && attempts < 1000) {
        x = rand() % map->width;
        z = rand() % map->height;
        if ((int)avoidPos.x == x && (int)avoidPos.y == z) continue;
        if (IsTileOccupiedByEnemy(x, z)) continue;
        found = true;
        break;
    }
    if (!found) return;
    ...
    int idx = -1;
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (!enemies[i].alive) { idx = i; break; }
    }
    if (idx == -1) return;
    ...
    enemies[idx].health = 3 + rand() % 3; // 3-5 HP
    enemies[idx].maxHealth = enemies[idx].health;
...
bool IsTileOccupiedByEnemy(int tx, int tz) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (!enemies[i].alive) continue;
        if ((int)enemies[i].pos.x == tx && (int)enemies[i].pos.y == tz) return true;
    }
    return false;
}
...
int GetEnemyAtTile(int tx, int tz) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (!enemies[i].alive) continue;
        if ((int)enemies[i].pos.x == tx && (int)enemies[i].pos.y == tz) return i;
    }
    return -1;
}