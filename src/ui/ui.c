...
void DrawUI(UIState* ui, GameState state) {
    ...
    if (state == IDLE) {
        int btnStartX = (GetScreenWidth() - (3 * BUTTON_WIDTH + 2 * BUTTON_SPACING)) / 2;
        DrawRectangle(btnStartX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, ui->meleeHover ? (Color){220,220,240,255} : (Color){70,70,90,255});
        ...
        DrawRectangle(rangedX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, ui->rangedHover ? (Color){220,220,240,255} : (Color){70,70,90,255});
        ...
        DrawRectangle(spellX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, ui->spellHover ? (Color){220,220,240,255} : (Color){70,70,90,255});
        ...
        int cancelX = (GetScreenWidth() - BUTTON_WIDTH) / 2;
        DrawRectangle(cancelX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT, ui->cancelHover ? (Color){255,200,200,255} : (Color){200,70,70,255});
...
bool IsCancelClicked(UIState* ui, Vector2 mouse, GameState state) {
    ...
    if (mouse.y < PANEL_Y) return false;
    int cancelX = (GetScreenWidth() - BUTTON_WIDTH) / 2;
    Rectangle cancelRect = { cancelX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT };
...
bool IsModeButtonClicked(UIState* ui, Vector2 mouse, GameState state, GameState* newMode) {
    ...
    int btnStartX = (GetScreenWidth() - (3 * BUTTON_WIDTH + 2 * BUTTON_SPACING)) / 2;
    Rectangle meleeRect = { btnStartX, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT };
    if (CheckCollisionPointRec(mouse, meleeRect)) { *newMode = MELEE; return true; }
    Rectangle rangedRect = { btnStartX + BUTTON_WIDTH + BUTTON_SPACING, BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT };
    if (CheckCollisionPointRec(mouse, rangedRect)) { *newMode = RANGED; return true; }
    Rectangle spellRect = { btnStartX + 2 * (BUTTON_WIDTH + BUTTON_SPACING), BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT };
    if (CheckCollisionPointRec(mouse, spellRect)) { *newMode = SPELL; return true; }
    return false;
}